...
def factorialize(number):
    ...
    """ build list of numbers from given number """
    result_list = []
...
def largest_number_in_arrays(array_of_numbers):
    """
    Given an array of arrays of numbers, return the largest.
    """
    found_in_array = []
    if (len(array_of_numbers)):
        for index, array in enumerate(array_of_numbers):
            largest = array[0]
            for element in array:
                if element > largest:
                    largest = element
            found_in_array.append(largest)
...
def confirm_ending(string, target):
    ...
    """
    matched = False
    if string[-len(target):len(string)] == target:
        matched = True
    return matched


def toddler(string, x_times):
    """
...
def truncate_string(string, position):
    """
    ...
    if len(string) <= position: 
        ...
        truncated_string = string[0:position] + "..."
    return truncated_string


def finders_keepers(array, truth_test):
    """
    ...
    """
    for element in array:
        if truth_test(element):
            return element


def slice_n_splice(array_one, array_two, index):
    """
    ...
    """
    new_array = []
    for indice, element in enumerate(array_two):
        if indice == index:
            for element2 in array_one:
                new_array.append(element2)
        new_array.append(element)
    return new_array


def getIndexToIns(array, value):
    """
